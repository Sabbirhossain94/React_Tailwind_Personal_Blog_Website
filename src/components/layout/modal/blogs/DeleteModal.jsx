...
function DeleteModal({ isOpen, setIsOpen, refetch, singleBlogId, selectedBlogId, setSelectedBlogId }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            if (selectedBlogId.length > 0) {
                await deleteMultipleBlogs(selectedBlogId)
            } else {
                await deleteBlogs(singleBlogId);
            }
        } catch (error) {
            console.error(error)
        } finally {
